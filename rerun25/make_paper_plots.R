...
mm_paper_error_plot <- function(data, unit, measure, error, 
  ...
  if (measure == 'occmean') {
    plt <- plt + theme(legend.position = c(0.50, 0.80),
...
mm_paper_error_diff_plot <- function(data, unit, measure, error, 
                               title, subtitle, note=NULL,
                               ylim_ub=NA) {
  ...
  if (measure == 'occmean') {
    plt <- plt + theme(legend.position = c(0.30, 0.53),
                       legend.background = element_blank(),
                       legend.box.background = element_rect(colour = "black"))
  }  else {
    plt <- plt + theme(legend.position = "none")
