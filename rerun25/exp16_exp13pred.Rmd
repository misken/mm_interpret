...
library(tidyr)
```
# exp16 - small scenarios space within, but not on, exp13 space
...
```{r read_results}
pred_sim_exp16 <- read.csv("exp16/predictions_simulated_exp16_long.csv", stringsAsFactors = TRUE)

inputs_exp16 <- read.csv("exp16/exp16_obflowsim_scenario_inputs.csv", stringsAsFactors = TRUE)

table(pred_sim_exp16$qdata, pred_sim_exp16$model)

```

```{r}
# Reorder factors
pred_sim_exp16$qdata <- factor(pred_sim_exp16$qdata, levels = c("noq", "basicq", "q", "onlyq"))
...
```{r}
pred_sim_exp16$model <- recode_factor(pred_sim_exp16$model, 
                                      lassocv="LASSO",
                                      lm="LM",
                                      poly="POLY",
                                      rf="RF",
...
pred_sim_exp16$measure_nice <- recode_factor(pred_sim_exp16$measure, 
...
pred_sim_exp16$unit_nice <- recode_factor(pred_sim_exp16$unit, 
                                      obs="Observation",
                                      ldr="LDR",
                                      pp="Postpartum"
                                      )

pred_sim_exp16$qdata_nice <- recode_factor(pred_sim_exp16$qdata, 
                                      noq="None (NQ)",
                                      basicq="Basic (BQ)",
                                      q="Advanced (AQ)",
                                      onlyq="Only (OQ)"
...
```{r merge}
input_output_exp16 <- inner_join(pred_sim_exp16, inputs_exp16, by=c('scenario'))
```
...
```{r reshape}
input_output_exp16_long <- input_output_exp16 %>% 
...
```{r}
exp13p16_obs_probblocked_lasso_pvsa <- input_output_exp16_long %>% 
  filter(unit == 'obs',
         qdata %in% c('q'),
         arrival_rate == 0.5,
         cap_pp == 24,
...
ggsave('exp16/paper_pvsa_plots/exp13p16_obs_probblocked_lasso_pvsa.png')
```

```{r}
exp13p16_ldr_probblocked_lasso_pvsa <- input_output_exp16_long %>% 
  filter(unit == 'ldr',
         qdata %in% c('q', 'noq'),
         arrival_rate == 0.5,
         cap_ldr == 12,
...
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_probblocked_lasso_pvsa.png')
```
```{r}
exp13p16_ldr_condmeantimeblocked_lasso_pvsa <- input_output_exp16_long %>% 
  filter(unit == 'ldr',
         qdata %in% c('q', 'noq'),
         arrival_rate == 0.5,
         cap_ldr == 12,
...
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_condmeantimeblocked_lasso_pvsa.png')
```

```{r}
exp13p16_ldr_probblocked_rf_pvsa <- input_output_exp16_long %>% 
  filter(unit == 'ldr',
         qdata %in% c('q', 'noq'),
         arrival_rate == 0.5,
         cap_ldr == 12,
...
exp13p16_ldr_probblocked_pvsa
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_probblocked_rf_pvsa.png')
```


```{r}
exp13p16_ldr_condmeantimeblocked_rf_pvsa <- input_output_exp16_long %>% 
  filter(unit == 'ldr',
         qdata %in% c('q', 'noq'),
         arrival_rate == 0.5,
         cap_ldr == 12,
...
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_condmeantimeblocked_rf_pvsa.png')
```

```{r}
exp13p16_pp_occmean_pvsa <- input_output_exp16_long %>% 
  filter(unit == 'pp',
         qdata %in% c('basicq'),
         arrival_rate == 0.5,
         cap_ldr == 12,
...
ggsave('exp16/paper_pvsa_plots/exp13p16_pp_occmean_pvsa.png')
```
...
exp13p16_ldr_occmean_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'ldr',
         qdata != 'onlyq', 
...
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_occmean_pvsa.png')
```

```{r exp13p16_ldr_occp95_pred_vs_act}
exp13p16_ldr_occp95_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'ldr',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'occp95') %>% 
...
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_occp95_pvsa.png')
```

```{r exp13p16_ldr_probblocked_pred_vs_act}
exp13p16_ldr_probblocked_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'ldr',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'probblocked') %>% 
...
exp13p16_ldr_probblocked_pvsa
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_probblocked_pvsa.png')
```

```{r exp13p16_ldr_condmeantimeblocked_pred_vs_act}
exp13p16_ldr_condmeantimeblocked_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'ldr',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'condmeantimeblocked') %>% 
...
exp13p16_ldr_condmeantimeblocked_pvsa
ggsave('exp16/paper_pvsa_plots/exp13p16_ldr_condmeantimeblocked_pvsa.png')
```

## PP

```{r exp13p16_pp_occmean_pred_vs_act}
exp13p16_pp_occmean_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'pp',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'occmean') %>% 
...
exp13p16_pp_occp95_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'pp',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'occp95') %>% 
...
ggsave('exp16/paper_pvsa_plots/exp13p16_pp_occp95_pvsa.png')
```

## OBS

```{r exp13p16_obs_occmean_pred_vs_act}
exp13p16_obs_occmean_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'obs',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'occmean') %>% 
...
ggsave('exp16/paper_pvsa_plots/exp13p16_obs_occmean_pvsa.png')
```


```{r exp13p16_obs_occp95_pred_vs_act}
exp13p16_obs_occp95_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'obs',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'occp95') %>% 
...
ggsave('exp16/paper_pvsa_plots/exp13p16_obs_occp95_pvsa.png')
```


```{r exp13p16_obs_probblocked_pred_vs_act}
exp13p16_obs_probblocked_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'obs',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'probblocked') %>% 
...
ggsave('exp16/paper_pvsa_plots/exp13p16_obs_probblocked_pvsa.png')
```


```{r exp13p16_obs_condmeantimeblocked_pred_vs_act}
exp13p16_obs_condmeantimeblocked_pvsa <- pred_sim_exp16 %>% 
  filter(unit == 'obs',
         qdata != 'onlyq', 
         model %in% c('LASSO', 'LM', 'NNET', 'RF'),
         measure == 'condmeantimeblocked') %>% 
...
ggsave('exp16/paper_pvsa_plots/exp13p16_obs_condmeantimeblocked_pvsa.png')
```
...
ggsave('exp16_predvact_ldr_occmean.png')
```
...
ggsave('exp16_predvact_ldr_occmp95.png')
```
...
ggsave('exp16_predvact_ldr_probblocked.png')
```
...
ggsave('exp16_predvact_ldr_condmeantimeblocked.png')
```
